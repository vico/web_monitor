...
def attrib():
    ...
    exposureGraphDf = tExposureDf[:, param_adviser].unstack().reindex(tExposureDf.index.levels[0]).dropna() #.fillna(method='ffill')
    exposure_graph = [{
    ...
    position_table = position_table.merge(positionPl, left_index=True, right_index=True, how='outer'). \
    ...
    positionSeries = position_table.sum()
